			 ...
			 unsigned int freq)
{
	...
	if (len < IEEE80211_HDRLEN + 2 + 1) {
        printf("RX_ACTION category - too short payload\n");
		hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211,
			       HOSTAPD_LEVEL_DEBUG,
		    ...
		    struct hostapd_frame_info *fi)
{
	...
	if (len < 24) {
        printf("RX_DEBUG frame was smaller than 24\n");
        return 0;
    }
	...
	if (is_multicast_ether_addr(mgmt->sa) ||
		...
		wpa_printf(MSG_DEBUG, "MGMT: Invalid SA=" MACSTR
			   ...
			   MAC2STR(mgmt->sa));
        printf("RX_DEBUG ignore this frame silently\n");
		return 0;
	}
    ...
    printf("RX_DEBUG frame received: %d (TYPE=%d)\n", len, stype);
	...
	case WLAN_FC_STYPE_ACTION:
		...
		break;
	default:
        printf("RX_DEBUG: unknown frame caught\n");
		hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211,
			       HOSTAPD_LEVEL_DEBUG,
