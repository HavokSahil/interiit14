					 ...
					 size_t reply_size)
{
    ...
    wpa_printf(MSG_DEBUG, "CUSTOM: test_begin");

    if (hwaddr_aton(cmd, addr)) {
        wpa_printf(MSG_DEBUG, "CUSTOM: weird-address");
        return -1;
    }
	pos = os_strchr(cmd, ' ');
    if (!pos) { 
        wpa_printf(MSG_DEBUG, "CUSTOM: seperator not found");
        return -1;
    }
	pos++;
	if (os_strncmp(pos, "req_mode=", 9) == 0) {
        ...
        if (val < 0) {    
            wpa_printf(MSG_DEBUG, "CUSTOM: hex-2-byte failed for req_mode");
            return -1;
        }
		req_mode = val;
		pos += 11;
    ...
    if (!req) {
        wpa_printf(MSG_DEBUG, "CUSTOM: wpabuf_parse_bin");
        return -1;
    }
    ...
    wpa_printf(MSG_DEBUG, "CUSTOM: The ret was %d", ret);
	return ret;
}
