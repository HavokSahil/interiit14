...
struct wpabuf * wpabuf_parse_bin(const char *buf)
{
	...
	len = os_strlen(buf);
    if (len & 0x01) {
        wpa_printf(MSG_DEBUG, "Invalid length of the message");
        return NULL;
    }
	len /= 2;
	...
	ret = wpabuf_alloc(len);
    if (ret == NULL) {
        wpa_printf(MSG_DEBUG, "Buffer length allocation failed");
        return NULL;
    }

	if (hexstr2bin(buf, wpabuf_put(ret, len), len)) {
		wpabuf_free(ret);
        wpa_printf(MSG_DEBUG, "msg: %s", buf);
        wpa_printf(MSG_DEBUG, "Hex string 2 binary conversion failed");
		return NULL;
	}
