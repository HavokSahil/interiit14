...
class HostapdController:
    def __init__(self, ctrl_path="/var/run/hostapd/wlan0", iface="wlan0", rxmux: RxMux = None):
        ...
        self._running = False
        self.rxmux: Optional[RxMux] = rxmux
        self.last_cmd_status = LastCommandStatus.NOINIT
    ...
    def handle_rx_msg(self, msg: str) -> bool:
        if "AP-MGMT-FRAME-RECEIVED" in msg and self.rxmux:
            Logger.log_info(f"handle_rx_msg: msg was {msg}")
            buf = RxMux.parse_buf_string(msg)
        ...
        if "AP-STA-DISCONNECTED" in msg:
            ...
            #TODO: sometimes the client roams without reporting, in that
            #case should I consider those as successful roaming, however
            #its true that it cannot be verified in that case.
            # clear the databases
    ...
    def _reader_loop(self):
        ...
        while self._running:
            ...
            if ready[0]:
                ...
                if msg.startswith('<'):
                    Logger.log_debug(f"reader_loop: EVENT: {msg}")
                    self.handle_rx_msg(msg)
                else:
                    Logger.log_debug(f"reader_loop: REPLY: {msg}")
                    if msg.strip() == "OK":
                        self.last_cmd_status = LastCommandStatus.OK
                    elif msg.strip() == "FAIL":
                        self.last_cmd_status = LastCommandStatus.FAIL
                    else:
