...
class NeighborRequestBuilder:
    ...
    def __init__(
        self,
        neighbor: Optional[NeighborInfo] = None,
    ...
    def parse(command: str) -> Dict[str, Any]:
        """
        Parse an existing 'SET_NEIGHBOR' string into readable fields.
        Only decodes the fields generated by this class.
        """
        if not command.startswith("SET_NEIGHBOR"):
            raise ValueError("Invalid command")

        parts = command.split()
        bssid = parts[1]

        result = {"bssid": bssid, "stationary": "stat" in parts}
        kv_pattern = re.compile(r'(\w+)=("[^"]*"|\S+)')
        for key, val in kv_pattern.findall(command):
            clean = val.strip('"')
            if key == "ssid":
                result["ssid"] = clean
            elif key == "nr":
                ...
                result["lci_raw"] = clean
            elif key == "civic":
                result["civic_raw"] = clean
            elif key == "bss_parameter":
                result["bss_parameter"] = int(clean)
    ...
    @staticmethod
    def _quote_ssid(ssid: str) -> str:
        """Legacy method for backward compatibility (not used in build())."""
        if re.search(r'\s', ssid) or '"' in ssid:
            ssid = ssid.replace('"', '\\"')
            return f'"{ssid}"'
        return ssid
