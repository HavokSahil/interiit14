"""
FTM RTT-enabled datatypes for wireless simulation.
Extended from multi-timescale-controller/datatype.py with FTM capability.
"""
...
class AccessPoint:
    ...
    obss_pd_violation_history: deque = field(default_factory=lambda: deque(maxlen=100))
    roam_in_rate: float = 0.0  # Rate of clients roaming into this AP (clients/step)
    roam_in_history: deque = field(default_factory=lambda: deque(maxlen=100))
    roam_out_rate: float = 0.0  # Rate of clients roaming out of this AP (clients/step)
    roam_out_history: deque = field(default_factory=lambda: deque(maxlen=100))
...
class Client:
    ...
    throughput_mbps: float = 0.0  # actual allocated throughput
    ...
    # FTM RTT (802.11mc) specific fields
    ftm_capable: bool = True  # Whether client supports 802.11mc FTM RTT
    measured_distances: Dict[int, float] = field(default_factory=dict)  # AP_ID -> distance in meters
    packet_error_rate: float = 0.0  # Packet Error Rate as percentage (0-100)
...
class Tile:
    ...
    # Status
    is_interference_hotspot: bool = False
