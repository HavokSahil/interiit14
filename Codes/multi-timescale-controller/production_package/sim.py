...
def calculate_qoe(row):
    """Calculate QoE score."""
    edge_tput = row['state_edge_p10_throughput']
    avg_tput = row['state_avg_throughput']
    retry_rate = row['state_p95_retry_rate'] * 100
    per = row['state_p95_per'] * 100
    rssi = row['state_median_rssi']
...
def run_inference(input_path, output_path, model_dir, config_path):
    ...
    start_time = datetime(2025, 12, 1, 15, 6, 47)
    ...
    for idx, row in df.iterrows():
        ...
        timestamp = (start_time + timedelta(seconds=idx)).isoformat(timespec='milliseconds')
